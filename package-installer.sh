...
function install_gcloud() {
    ...
    if !  sudo apt-get update && sudo apt-get install -y apt-transport-https ca-certificates gnupg &> /dev/null; then
    ...
    if ! ( sudo apt-get update && sudo apt-get install -y google-cloud-sdk=296.0.1-0 &> /dev/null ); then
...
if ! ( sudo apt-get update ); then
    ERROR "Apt failed to update packages list"
fi
...
for pkg in "${pkg_list[@]}"; do
    if [[ "${pkg}" == "gcloud" ]]; then
        ...
        if ! which "${pkg}" &> /dev/null; then
            INFO "'${pkg}' package not found. Installing .."
            if ! sudo apt-get install "${pkg}" -y 1> /dev/null; then
