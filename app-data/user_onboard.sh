...
function onboard(){
    ...
    USER=$(echo $(gcloud config list --format=json | jq -r '.core.account'))
    MEMBER="user:${USER}"
    echo -n "Does ${USER} have roles/owner or roles/iam.securityAdmin persmissions in the monitoring project ${MONITOR_PROJ_ID}? [Y/N]: ";
    read;
    role_cnf=${REPLY}
    ...
    if [ ${role_cnf} == 'Y' ]; then
        ...
        INFO "Checking whether ${USER} have valid persmission..."
        if ! gcloud projects get-iam-policy $MONITOR_PROJ_ID \
            ...
            INFO "Error while checking for owner role for ${USER} under ${MONITOR_PROJ_ID}"; exit 1
        ...
        if [ "$value1" == "" ]; then
            if ! gcloud projects get-iam-policy $MONITOR_PROJ_ID \
                ...
                ERROR "Error while checking for security admin role for ${USER} under ${MONITOR_PROJ_ID}"; exit 1
            else
        ...
        if [[ "$value1" == "user:$USER" || 
            ...
            ERROR "${USER} have no owner and security admin roles in ${MONITOR_PROJ_ID}"; exit 1
        fi
    elif [ ${role_cnf} == 'N' ]; then
        ERROR "Please provide the Onwer or Security Admin role for the user ${USER} in ${MONITOR_PROJ_ID}"; exit 1
    else
...
function role_admin(){
    ...
    INFO "Assigning RoleAdmin for $USER in $MONITOR_PROJ_ID"

    if ! gcloud projects $IAM $MONITOR_PROJ_ID --member=user:$USER --role=$ROLE &> /dev/null; then
        INFO "Unable to change roleAdmin for Monitoring project"
    else
        INFO "Successfully '$TASK' Role Admin permissions for $USER in $MONITOR_PROJ_ID"
    fi
}
