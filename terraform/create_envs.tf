...
resource "null_resource" "create_kube_envs" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      echo ROOT_RESOURCE_ID=projects/${var.forseti_projectid} >> ${var.kube_env_files_path}
      ...
      echo PATROL_FORSETI_SERVER_TAG=${var.PATROL_FORSETI_REST_VERSION} >> ${var.kube_env_files_path}
      ...
      echo PATROL_FORSETI_CREDENTIALS=./keys/${var.forseti_service_account_id}.json >> ${var.kube_env_files_path}
      ...
      echo PATROL_GRAFANA_VERSION="${var.PATROL_GRAFANA_VERSION}"  >> ${var.kube_env_files_path}
...
resource "null_resource" "create_enforcer_envs" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      echo FORSETI_API_SERVER=${var.PATROL_FORSETI_API_SERVER} >> ${var.docker_enforcer_env_files_path}
    EOT
  }
}

resource "null_resource" "create_forseti_envs" {
    triggers = {
      data="${timestamp()}"
    }
  provisioner "local-exec" {
    command = <<EOT
      truncate -s 0 ${var.docker_forseti_env_files_path}
      echo GOOGLE_APPLICATION_CREDENTIALS=/keys/${var.forseti_service_account_id}.json >> ${var.docker_forseti_env_files_path}
      echo BUCKET=gs://${module.create_patrol_scanner_bucket.name}  >> ${var.docker_forseti_env_files_path}
      echo GCP_ORGANIZATION=${var.patrol_forseti_gcp_organization} >> ${var.docker_forseti_env_files_path}
      echo PROJECT_ID=${var.patrol_projectid}  >> ${var.docker_forseti_env_files_path}
      echo SUBSCRIPTION_NAME=${module.create_patrol_forseti_subscription.name}  >> ${var.docker_forseti_env_files_path}
      echo FORSETI_SERVER_CONF=${var.PATROL_FORSETI_SERVER_CONF}  >> ${var.docker_forseti_env_files_path}
      echo FORSETI_API_SERVER=${var.PATROL_FORSETI_API_SERVER}  >> ${var.docker_forseti_env_files_path}
    EOT
  }
}
...
resource "null_resource" "create_apiserver_envs" {
    ...
    echo SCANNER_TOPIC_NAME=${module.create_patrol_forseti_pubsub_topic.name} >> ${var.docker_apiserver_env_files_path}
    ...
    echo FORSETI_SERVER_CONF=${var.PATROL_FORSETI_SERVER_CONF} >> ${var.docker_apiserver_env_files_path}
    ...
    echo GSUITE_SUPER_ADMIN_EMAIL=${var.patrol_apiserver_gsuite_super_admin_email} >> ${var.docker_apiserver_env_files_path}
    echo FORSETI_REST_SERVER=${var.PATROL_FORSETI_REST_SERVER} >> ${var.docker_apiserver_env_files_path}
    echo FORSETI_ENFORCER_SERVER=${var.PATROL_ENFORCER_SERVER} >> ${var.docker_apiserver_env_files_path}
    ...
    echo FORSETI_SA=${module.create_forseti_service_account.email} >> ${var.docker_apiserver_env_files_path}
...
resource "null_resource" "create_cloudsql_cred_envs" {
  ...
  provisioner "local-exec" {
    ...
    echo FORSETI_API_DB_NAME=${module.create_patrol_apiserver_database.name} >> ${var.docker_cloudsql_cred_env_files_path}
    echo FORSETI_API_DB_USERNAME=${module.create_patrol_apiserver_user.name} >> ${var.docker_cloudsql_cred_env_files_path}
    echo FORSETI_API_DB_PASSWORD=${module.create_patrol_apiserver_user.password} >> ${var.docker_cloudsql_cred_env_files_path}
    echo FORSETI_DB_NAME=${module.create_patrol_forseti_database.name}  >> ${var.docker_cloudsql_cred_env_files_path}
    echo FORSETI_DB_USERNAME=${module.create_patrol_forseti_user.name}  >> ${var.docker_cloudsql_cred_env_files_path}
    echo FORSETI_DB_PASSWORD=${module.create_patrol_forseti_user.password} >> ${var.docker_cloudsql_cred_env_files_path}
    EOT
  }
}
...
resource "null_resource" "create_event_trigger_envs" {
  ...
  provisioner "local-exec" {
    ...
    echo FORSETI_API_SERVER=${var.PATROL_FORSETI_API_SERVER}  >> ${var.docker_eventtrigger_env_files_path}
    echo FORSETI_EVENT_TRIGGER_TOPIC="${var.event_trigger_topic_name}" >> ${var.docker_eventtrigger_env_files_path}
    echo FORSETI_EVENT_TRIGGER_SUBSCRIPTION="${var.event_trigger_subscription_name}" >> ${var.docker_eventtrigger_env_files_path}
...
resource "null_resource" "create_stats_envs" {
  ...
  provisioner "local-exec" {
    ...
    echo FORSETI_API_SERVER=${var.STATS_FORSETI_API_SERVER} >> ${var.docker_stats_env_files_path}
    ...
    echo FORSETI_API_DB_NAME="${module.create_patrol_apiserver_database.name}" >> ${var.docker_stats_env_files_path}
    echo FORSETI_API_DB_USERNAME="${module.create_patrol_apiserver_user.name}" >> ${var.docker_stats_env_files_path}
    echo FORSETI_API_DB_PASSWORD="${module.create_patrol_apiserver_user.password}" >> ${var.docker_stats_env_files_path}
...
resource "null_resource" "create_grafana_envs" {
      triggers = {
      data="${timestamp()}"
    }
  provisioner "local-exec" {
    command = <<EOT
    truncate -s 0 ${var.docker_grafana_env_files_path}
    echo GF_SERVER_DOMAIN=${var.patrol_ip_domain} >> ${var.docker_grafana_env_files_path}
    echo GF_SERVER_ROOT_URL="%(protocol)s://%(domain)s/grafana" >> ${var.docker_grafana_env_files_path}
    echo GF_SERVER_FROM_SUB_PATH=true >> ${var.docker_grafana_env_files_path}
    echo GF_USERS_DEFAULT_THEME=light >> ${var.docker_grafana_env_files_path}
    echo GF_AUTH_DISABLE_LOGIN_FORM=true >> ${var.docker_grafana_env_files_path}
    echo GF_AUTH_ANONYMOUS_ENABLED=true >> ${var.docker_grafana_env_files_path}
    echo GF_AUTH_ANONYMOUS_ORG_ROLE=Viewer >> ${var.docker_grafana_env_files_path}
    EOT
  }
}
