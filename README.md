...
## 1.  Prerequisites
One Ubuntu 18.04 machine (bare metal / VM), which is used for performing Biarca Patrol installation.
- A minimum of one GCP Project
- zip & unzip packages
- Terraform v0.12.3
- Gcloud v253.0.0 
- git
...
On the installer machine, install the below packages, if already not available.
### 1.1  Installing zip package
...
```
### 1.2 Installing Terraform v0.12.3
...
```
### 1.5 Installing JSON Processor jq on the Installer Machine
...
```
### 1.8  Installing Python version 2.7
...
Installation can be performed in one of the following two setup types.
###### Setup#1 - Install Biarca Patrol setup on one dedicated project and then monitor other project(s)
###### Setup#2 - Use the same project to install Biarca Patrol setup and also monitor the same
Note :- From here on, rest of the documentation refers to the above mentioned setup types, wherever applicable.

In the installer machine, create a folder (say patrol-installer) and navigate to that folder.
At a high level, Biarca Patrol installation is a 9 step procedure.
- Cloning Biarca Patrol Repository
- Creating Service Account
- Creating External Static Address and DNS Record
- Updating Configuration File
- Installing Patrol Specific GCP Resources
- Enabling IAP
- Update Service Accounts

### 2.1 Cloning Biarca Patrol Repository
Clone the git repository by using the below commands:
````
$ git clone https://github.com/Biarca/patrol-k8s-marketplace.git
````
### 2.2 Creating Service Account
Perform one of the below actions in GCP.
- If installation is performed for **Setup#1**, then create a service account and assign the below roles.
...
- If installation is performed for **Setup#2**, then create a service account with Project Owner role for that project.
...
### 2.3 Creating External Static Address and DNS Record
For accessing Biarca Patrol UI using an FQDN, perform the below steps in GCP.
#### 2.3.1 Create External Static IP
...
#### 2.3.2 Create a DNS Record for Reserved External Static IP
...
### 2.4 Below is the list of parameters in **installer_envs** file, which needs to be updated with user specific info, before continuing with the installation.
...
**Note:** For Setup Type #2, values for ***PATROL_PROJECTID*** & ***MONITOR_PROJECTID*** are same.
### 2.5 Creating Patrol Specific GCP Resources
...
$ bash installer.sh
````
Executing this script would perform the below tasks.
#### 2.5.1 Enable GCP APIs
Installer will enable the below APIs for installer project, irrespective of the installation setup type (Setup#1 Or Setup#2)
...
#### 2.5.2 Create GCP Resources
Below resources will be created as part of script execution
- Service Accounts - 5
- Firewall Rules
- Buckets - 2
- Pubsub topics - 3
...
`Note: Post successful completion of the script, few values will be displayed at the end of the script. Those values MUST be provided in the Marketplace UI during the Patrol app installaton`
### 2.6 Patrol Installation from GCP Marketplace
Follow the below steps to install the patrol application
Go to GCP Console, click on 'Configure' and follow the on-screen instructions. Provide the input values genearted at the end of script from the previous section to populate the form.
...
Click on ````HTTPS RESOURCES```` tab and Enable the toggle button beside the entry ````patrol-ui-<random id>````(defined in section #7.5 above) to enable IAP for UI service. A pop up window is displayed.
...
### 3.2 Update Service Accounts
Post successful installation of Biarca Patrol, in the installer project execute the below to remove the Owner role for the  installer and monitor service accounts.
````
$ cd <Path to patrol-installer>/ biarca-patrol-terraform/
$ bash remove_serviceaccount_roles.sh
````

## UnInstall Patrol Application:
